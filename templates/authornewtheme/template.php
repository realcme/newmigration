 ...
 *   A sequential array of variables to pass to the theme template.
 */
function authornewtheme_preprocess_page(&$vars) {
  //$vars['sample_variable'] = t('Lorem ipsum.');
  $theme_path = base_path() . path_to_subtheme();

  // here we are going to strip away old jQuery and add in the latest one
	$scripts = array();
	foreach (explode("\n", $vars['scripts']) as $script) {
		// old jquery appears, replace it with latest jquery (2 lines of html)
		if (0 < strpos($script, '/cms/misc/jquery.js')) {
                        // Fix for responsive glitch on Safari 9.0.3; see ticket #8256 http://dev.realcme.com/trac/ticket/8256
			//$scripts[] = str_replace('/cms/misc/jquery.js', "$theme_path/js/jquery-1.11.1.js", $script);
			$scripts[] = str_replace('/cms/misc/jquery.js', "//code.jquery.com/jquery-1.11.1.min.js", $script);

                        $scripts[] = str_replace('/cms/misc/jquery.js', "//code.jquery.com/ui/1.11.1/jquery-ui.min.js", $script);
                    
                        $scripts[] = str_replace('/cms/misc/jquery.js', "$theme_path/js/jquery-migrate-1.2.1.min.js", $script);
			// we don't need jquery_update stuff either
		} else if (strpos($script, '/cms/sites/all/modules/jquery_update/compat.js') == 0 /*&&
							 strpos($script, '/cms/sites/all/modules/custom/cmebase/js/allscripts.js') == 0*/ ) {
			$scripts[] = $script;
		}
	}
	$vars['scripts'] = implode("\n", $scripts);

	// 	$vars['scripts'] = str_replace('/cms/misc/jquery.js', "$theme_path/js/jquery-1.11.1.js", $vars['scripts']);
	$vars['scripts'] = str_replace(base_path().drupal_get_path('module', 'og').'/og_access.js', "$theme_path/js/og_access-fixed.js", $vars['scripts']);

  $scr = "<script>\n"
  	. "$.preload = $.noop;\n"
  	. "tb_show = $.noop;\n"
  	. "tb_remove = function() { $('#popup').hide(); $('#popup > .wrapper > iframe').attr('src', ''); };\n"
  	//. "getWindowHeight = function() { return $(window).height(); }\n"
  	. "</script>\n"
        .'<script type="text/javascript">$(window).load(function(){getWindowHeight = $(this).height();});</script>'
 		. "<script type='text/javascript' src='$theme_path/js/jquery.cookie.js'></script>\n"
 		. "<script type='text/javascript' src='$theme_path/js/author.js'></script>\n";
...
function theme_views_view_table_authoractivities($view, $nodes, $type) {
    ...
    foreach ($view->field as $field) {
      ...
      if ($field['field'] == 'type') {
        ...
        if ($prepub_url) {
          ...
          $data .= "<a href='$prepub_url'><button>View Published</button></a>";
        } else {
          $data .= "<a class='invisible' href='$prepub_url'><button>View Published</button></a>";
        }
        if ($can_import) {
  ...
  array_pop($view->table_header); // remove last 2 columns
  array_pop($view->table_header); // remove last 2 columns
...
/**
 * Implement header theming
 */
function phptemplate_header($data = NULL) {
	//$theme_path = base_path() . drupal_get_path('theme', 'author');
	//drupal_add_js($theme_path . '/js/author.js');
	return _phptemplate_callback('header', array('data' => $data), array('header'));
}

function phptemplate_footer($data = NULL) {
	//$theme_path = base_path() . drupal_get_path('theme', 'author');
	//drupal_add_js($theme_path . '/js/author.js');
	return _phptemplate_callback('footer', array('data' => $data), array('footer'));
}

function phptemplate_list_courses($data = NULL) {
	drupal_add_js(path_to_subtheme().'/js/listing.js');
	return _phptemplate_callback('list_courses', array('data' => $data), array('list_courses'));
}

function phptemplate_list_templates($data = NULL) {
	drupal_add_js(path_to_subtheme().'/js/listing.js');
	return _phptemplate_callback('list_templates', array('data' => $data), array('list_templates'));
}
