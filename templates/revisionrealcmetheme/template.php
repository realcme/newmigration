<?php
// $Id: template.php,v 1.3 2009-04-03 15:37:53 valerie Exp $
...
function theme_views_view_table_authoractivities($view, $nodes, $type) {
  ...
  foreach ($nodes as $node) {
    ...
    foreach ($view->field as $field) {
      if ($field['label'] == 'Type') {
        ...
        $course_type_prefix = str_replace(' ', '', strtolower($data));
        ...
        // The course status is stored in the course ... Find a better way to join...
        $fm_id = $node->$field['queryname'];
        $fmnode = node_load($fm_id);
        $course_id = $fmnode->og_groups[0];
        $course = node_load($course_id);
        if ($course->field_activity_status[0]['value'] == 10) {
    ...
    foreach ($view->field as $field) {
      if ($fields[$field['id']]['visible'] !== FALSE) {
        ...
        $data = views_theme_field('views_handle_field', $field['queryname'], $fields, $field, $node, $view);
        if ($field['label'] == 'Type') {
           //identify for style to hide
          $cell['data'] = $coursetype;
          $cell['valign'] = 'middle';
          $cell['align'] = 'center';
          $row['data'][] = $cell;  
        } else if ($field['field'] == 'field_course_title_value') {
          $cell['data'] = l($data, 'node/'.$fm_id);
          $cell['valign'] = 'middle';
          $row['data'][] = $cell;  
        } else if ($field['field'] == 'parent') {
          $data = '<img src="'.$img_base_path.'course_status_'.$course_status.'.gif" alt="'.$course_status.'" title="'.$course_status.'" />';
          $cell['data'] = $data;
          $cell['align'] = 'center';
          $row['data'][] = $cell;
        } else {
  ...
  $url = base_path().'cmeauthor/new_tp_course';
  if ($view->total_rows > $view->nodes_per_page) {
    // pager
    $output .= "<a href='$url'><div id='author-new-activities'></div></a>";
  } else {
    // no pager
    $output .= "<div class='nopager'><a id='author-new-activities' href='$url'></a></div>";
  }
  return $output;
}
...
function zen_username($object) {
  if ($object && $uid = $object->uid) {  
    $user = user_load(array('uid' => $uid));
    return theme('user_public_name', $user);
  } else {
    return '';
  }
}
