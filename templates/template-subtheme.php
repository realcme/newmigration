<?php
// $Id: template-subtheme.php,v 1.1 2008-04-18 02:24:21 valerie Exp $
 ...
 **************************
 */
drupal_add_css(path_to_theme() .'/listing.css', 'theme', 'all');
if (file_exists(path_to_subtheme() .'/listing.css')) {
	drupal_add_css(path_to_subtheme() .'/listing.css', 'theme', 'all');
}

function theme_views_view_table_listing_all($view, $nodes, $type) {
  require_once(drupal_get_path('module', 'cmemembership').'/view-course-listing.inc');
  return activity_listing_all($view, $nodes, $type);
}

function theme_views_view_table_listing_curriculum($view, $nodes, $type) {
  require_once(drupal_get_path('module', 'cmemembership').'/view-course-listing.inc');
  return activity_listing_curriculum($view, $nodes, $type);
}

function theme_views_view_table_listing_archived($view, $nodes, $type) {
  require_once(drupal_get_path('module', 'cmemembership').'/view-course-listing.inc');
  return activity_listing_all($view, $nodes, $type);
}
