<?php
// $Id: template.php,v 1.3 2009-02-18 18:02:52 palle Exp $
...
drupal_add_css(path_to_subtheme() .'/brand.css', 'theme', 'all');
 ...
 * 
function zen_header_title($nid) {
  $node = node_load($nid);
  if ($node) {
    $header_title = "CME Library";
    if ($node->field_header_title[0]['value']) {
      $header_title = $node->field_header_title[0]['value'];
    } else if ($node->title) {
      $header_title = $node->title;
    }
    $trimmed_header_title = trim_pretty_string($header_title, 70, '...');
    drupal_add_js(fill_title_js($trimmed_header_title), "inline");
  }
  return "";
}

function fill_title_js($header_title) {
  $output = "
