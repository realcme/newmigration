(function ($) {
    ...
    Listing = {
        ...
        getParams: function ($listing, offset, max) {
            ...
            $listing.find('ul').fadeOut(function () {
                $(this).empty();
            });
...
//						}
                        ...
                        var $button = $('<a class="preview right">').attr({
                            title: 'View a Course Preview',
                            href: '/cms/node/' + e.prepbnid
                        }).html('View Course Preview');
                        if (!e.prepbnid) $button.css('visibility', 'hidden');
                        $item.append($button);

                        $button = $('<a class="delete right">').attr({
                            title: 'Delete Course',
                            href: '/cms/cmeauthor/delete_course/' + e.nid
                        });
                        if (e.status == 'published') $button.css('visibility', 'hidden');
                        $item.append($button);

                        $item.append($('<a class="edit right">').attr({
                            title: 'Edit Course',
                            href: '/cms/builder/' + e.fmnid
                        }));
                        $item.append($('<a class="title">').attr({
                            title: 'Edit Course',
                            href: '/cms/builder/' + e.fmnid
                        }).html(e.title));
            ...
            if ($listing.length) {
                ...
                $.post('/cms/cmeauthor/json_template_listing', params, function (data) {
                    ...
                    $.each(data.list, function (n, e) {
                        ...
                        if (embedded) {
                            if (new_course) {
                                $item.append($('<a class="new right">').attr({
                                    title: 'New Course from this Template',
                                    href: '/cms/builder/' + e.nid + '?v=c'
                                }).html('Create New Course'));
                            } else {
                                $item.append($('<a class="new right">').attr({
                                    title: 'Edit Template',
                                    href: '/cms/builder/' + e.nid + ''
                                }).html('Edit Template'));
                            }
                            $item.append($('<div class="title">').html(e.title));
                        } else {
                            var $button = $('<a class="new right">').attr({
                                title: 'New Course from this Template',
                                href: '/cms/builder/' + e.nid + '?v=c'
                            }).html('New Course from this Template');
                            if ($listing.hasClass('nocreate')) $button.css('visibility', 'hidden');
                            $item.append($button);
                            if (params.library == '0') {
                                $item.append($('<a class="delete right">').attr({
                                    title: 'Delete Template',
                                    href: '/cms/node/' + e.nid + '/delete?destination=templates'
                                }));
                            }
                            $item.append($('<a class="edit right">').attr({
                                title: 'EditTemplate',
